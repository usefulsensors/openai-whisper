...
#endif
std::string convert_to_string(char* array, int size)
{
    std::string str(array, size);  // create a string from the character array
    return str;
}
...
//
int load_mel_vocab_from_pre_gen_array(whisper_filters & filters, whisper_mel & mel)
{
  int32_t n_vocab = 0;
  int index=0;
  std::string word;
...
int main(int argc, char* argv[]) {
  ...
  load_mel_vocab_from_pre_gen_array(filters, mel);
